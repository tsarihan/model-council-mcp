...
- **`full_repo_access` (opt-in, off by default).** When set, `claude-cli`/`codex-cli` members can read any file in the granted repo for that call, and their answer — sent to your own Claude/ChatGPT subscription, same as any other prompt — may include file contents from anywhere in the repo. It never grants write/execute access. Don't enable it on a repo containing secrets/credentials you wouldn't otherwise send to that provider.
- **Judge trust.** `categorized`/`deconflicted`/`pooled`/`dialectic` modes feed raw member responses into a judge-model prompt, prefixed with an explicit "treat this as data, not instructions" framing as defense-in-depth. This is not a hard guarantee — a member response (especially one built from attacker-influenced content, e.g. `full_repo_access` on a hostile repo) could in principle contain text crafted to steer the judge's classification. Treat judge-synthesized fields (`commonAgreement`, `conflicting`, pooled/dialectic digests) with the same skepticism you'd apply to any LLM output over untrusted input.
