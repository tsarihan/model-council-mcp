...
| `CLAUDE_CLI_OLLAMA_MODELS` | Comma-separated Ollama model names to run through the `claude` CLI's own harness — see below. Unset = feature off. | — |
...
**Behavior & requirements**
- **These members are NOT Claude.** They're open-weight models answering through Claude Code's harness purely to get real tool use; `list_models`/`get_council_config` label them `"<model> (via claude CLI harness, <address>)"`, never `"Claude ..."`.
...
- Opt-in only: unset by default, and these members never join the zero-config auto-populated council even when Ollama cloud is otherwise reachable — you must add `CLAUDE_CLI_OLLAMA_MODELS` (or the equivalent `configure_council`/`COUNCIL_MODELS` entry) explicitly.
- Full repo contents can be sent to whichever Ollama backend answers — a **local** Ollama model keeps this fully offline, but a `:cloud` model sends repo contents to Ollama's cloud the same way `full_repo_access` sends them to Claude/ChatGPT's cloud for the first-party CLIs. Don't point this at a `:cloud` model in a repo containing secrets you wouldn't otherwise send there.
...
- **`full_repo_access` (opt-in, off by default).** When set, `claude-cli`/`codex-cli` members can read any file in the granted repo for that call, and their answer — sent to your own Claude/ChatGPT subscription, same as any other prompt — may include file contents from anywhere in the repo. It never grants write/execute access. Don't enable it on a repo containing secrets/credentials you wouldn't otherwise send to that provider. A `CLAUDE_CLI_OLLAMA_MODELS` member is enforced the same way but sends repo contents to whichever Ollama backend answers instead — a **local** Ollama model keeps this fully offline; a `:cloud` model sends it to Ollama's cloud.
